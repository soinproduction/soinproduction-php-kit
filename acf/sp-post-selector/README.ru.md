...
`taxonomy_terms` опционально ограничивает доступные и возвращаемые записи выбранными термами в формате `taxonomy:term_id`. Несколько термов одной таксономии объединяются как `IN`, а ограничения разных таксономий — как `AND`. Dropdown в picker показывает только разрешённые термы.
