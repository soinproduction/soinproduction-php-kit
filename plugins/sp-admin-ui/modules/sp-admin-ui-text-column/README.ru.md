...
Предоставляет `register_acf_text_column()` для текстовых колонок только для чтения в таблицах записей и терминов WordPress. Непустой `acf_field` выводит скалярное значение ACF. Пустой `acf_field` выводит нативный excerpt записи или description термина.
