...
Когда модуль и ускорение активны, в верхней панели WordPress появляется меню **SP Cache** с защищённой nonce-командой **Clear cache**. После очистки пользователь возвращается на исходную страницу, а автоматический прогрев запускается тем же событием, что и при очистке со страницы настроек.
...
- Critical font preload автоматически предпочитает обычные non-italic WOFF2-начертания разных текстовых семейств и пропускает icon fonts. Максимум настраивается (по умолчанию два файла), итоговый список меняется фильтром `sp_accelerator_preload_fonts`.
