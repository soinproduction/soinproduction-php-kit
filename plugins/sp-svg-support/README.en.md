...
The module is loaded by PHP Kit; there is no settings record and no admin page. Prefix `sp-svg-support` with `_` in the PHP Kit `plugins` configuration to disable it. Existing SVG attachments remain in the library, but new uploads and frontend transformations stop.
