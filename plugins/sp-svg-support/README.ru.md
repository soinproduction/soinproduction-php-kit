...
Модуль загружается через PHP Kit; собственной admin page и option у него нет. Для отключения добавьте `_` к имени `sp-svg-support` в секции `plugins` конфигурации PHP Kit. Существующие SVG останутся в Media Library, но новые uploads и frontend transformations прекратятся.
